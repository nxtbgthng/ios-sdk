...
// Macros
#define PH_URL(PATH) [PHGetBaseURL() stringByAppendingString:@#PATH]
#define PH_URL_FMT(PATH,FMT) [PHGetBaseURL() stringByAppendingFormat:@#PATH, FMT]
...
/**
 * Returns base URL that should be used for SDK requests to PlayHaven server.
 **/
NSString *PHGetBaseURL(void);

/**
 * Provides ability to change base URL used for SDK requests.
 **/
void PHSetBaseURL(NSString *inBaseURL);
...
NSError *PHCreateErrorWithTypeAndDescription(PHErrorType errorType, NSString *errorDescription);
