...
{
    PHPublisherContentRequest *request = [PHPublisherContentRequest existingRequestForApp:token secret:secret placement:placement.lowercaseString];
    if (!!request) {
...
- (id)initWithApp:(NSString *)token secret:(NSString *)secret placement:(NSString *)placement delegate:(id)delegate
{
    if ((self = [self initWithApp:token secret:secret])) {
        self.placement = placement.lowercaseString;
