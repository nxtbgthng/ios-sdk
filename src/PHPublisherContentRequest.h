 ...
 **/
@interface PHPublisherContentRequest : PHAdRequest <PHAPIRequestDelegate> {
    NSString       *_placement;
    BOOL            _animated;
 ...
 *
 * @param campaignID
 *  Campaign id is an optinal identifier assigned to a push notification sent from the server. It is
 *	passed as a separate field in in the push notification payload. This property should be set for
 *	a content request that is created as a result of incoming push notification.
 *
 * @return
 *   A PHPublisherContentRequest instance
 **/
 ...
 *  - The PlayHaven's server stores all custom dimensions received from each device.
